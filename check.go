...
func (method *methodType) Call(c *C) {
	if method == nil {
		return
	}
	...
	case func(*testing.T):
		f(c.T)
	default:
		c.Fatalf("bad signature for method %s: %T", method.Info.Name, method.Interface())
	}
}
...
func (runner *suiteRunner) run(t *testing.T) {
	t.Helper()
	t.Run(suiteName(runner.suite), func(t *testing.T) {
...
func (runner *suiteRunner) runTest(t *testing.T, test *methodType) {
	t.Helper()
